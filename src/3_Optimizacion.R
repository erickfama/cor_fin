### Optimization ###

# Librerias ----
source("./src/3_Optim_function.R")

# Lectura ----
egresos_per <- read_csv("./data/3_final/egresos19_per_clean.csv")
egresos_inc <- read_csv("./data/3_final/egresos19_inc_clean.csv")

# Metricas ----
eval_eg_per <- Optim(egresos_per, method_name = "Percepción", props_values = seq(0, 10, length.out = 21))
eval_eg_inc <- Optim(egresos_per, method_name = "Incidencia", props_values = seq(0, 20, length.out = 21))
eval_eg <- rbind(eval_eg_per, eval_eg_inc)

# Escritura ----
write.csv(eval_eg, "./data/2_interim/eval_eg.csv", row.names = FALSE)

# F1 score optim ----
eval_eg %>%
  filter(F1_score != 1) %>%
  ggplot(aes(prop, F1_score, color = method, label = prop)) +
  geom_point() +
  geom_line() +
  ggrepel::geom_text_repel(size = 4) +
  scale_y_continuous(limits = c(0, 1)) +
  facet_wrap(~ method) +
  labs(x = "Proporción") +
  theme_bw() + 
  theme(legend.position = "none")
